...
public class TeamC extends AbstractEvaluator {
    ...
    public int evaluateBoard(char [][] board) { 
        ...
        value=value*100;
        for (i=1;i<=4;i++) {
            if (super.ownChecker(board[i][1])) value=value+1;
            if (super.ownChecker(board[i][8])) value=value+1;
        }
        for (i=5;i<=8;i++) {
            if (super.oppChecker(board[i][1])) value=value-1;
            if (super.oppChecker(board[i][8])) value=value-1;
        }
        for (i=1;i<=8;i++) {
